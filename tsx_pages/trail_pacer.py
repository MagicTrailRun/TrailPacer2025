...
from TrailPacer.race_id import color_pente,altitude_metrics, process_data, load_data_checkpoints, create_col_profile, load_json,gpx_to_df
...
def show():
    ...
    with plan_course1:
        ...
        if track_file_gpx.exists():
            df_gpx = gpx_to_df(track_file_gpx)
            fig = plot_altitude_profile_area(df_gpx, df, config, affichages, target_time)
            st.plotly_chart(fig, use_container_width=True)

        elif track_file_json.exists():
            df_gpx = load_json(track_file_json)  # ✅ corrige ici
            fig = plot_altitude_profile_area(df_gpx, df, config, affichages, target_time)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with explorer3 : 
        if course == 'GRR':
            ...
            try :        
                # Charger les données
                # df_track = load_data_checkpoints(f"data/TrailPacer/{event}/{course}/checkpoints/checkpoints_{year}.csv")
                ...
                # df_timing = df_display[['checkpoint', f'temps_cumule_med_{target_time}']].copy()
                ...
                fig, metrics = create_col_profile(df_gpx, df, col_name=option)
